...
public:
  ...
  [[nodiscard]] string message(const int return_value) const noexcept override {
...
Address::Address(const string &node, const string &service,
  ...
  const int gai_ret =
      getaddrinfo(node.c_str(), service.c_str(), &hints, &resolvedaddress_);
  if (gai_ret != 0) {
  ...
  unique_ptr<addrinfo, decltype(addrinfo_deleter)> wrappedaddress_(
      resolvedaddress_, move(addrinfo_deleter));
...
pair<string, uint16_t> Address::ip_port() const {
  ...
  const int gni_ret =
      getnameinfo(address_, size_, ip.data(), ip.size(), port.data(),
